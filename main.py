...
async def cmd_del_club(message: Message, state: FSMContext):
    ...
    if my_clubs:
        await delete_from_bottable(my_clubs[0], message.chat.id)
            ...
            # if sub_result[event]['team1'] in user_clubs or sub_result[event]['team2'] in user_clubs:
                if type(sub_result[event]['team2']) not in [list]:
                    ...
                    if sub_result[event]['team1'].find(" / ") == -1:
                        ...
                        if (sub_result[event]['team1'] == c or sub_result[event]['team2'] == c) and m not in there_is:
                            part += m
                            cnt += 1
                            there_is.append(m)
                        # if sub_result[event]['team1'].startswith(c) or sub_result[event]['team2'].startswith(c):
                        part = await highlighted(part, c)
                        if len(part) >= 4000:
...
async def highlighted(event, team):
    pattern = r'(?<![-\w])' + re.escape(team) + r'(?![-\w])'
    return re.sub(pattern, f'<b>{team}</b>', event)
